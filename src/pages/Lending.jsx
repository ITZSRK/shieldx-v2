...
function ApplicationPipeline() {
  ...
  return (
    <div className="border border-white/10 rounded-xl bg-black/60 overflow-hidden font-mono text-xs">
          ...
          {STAGES.map((s, i) => (
            <div key={s.id} className="flex flex-col items-center" style={{ width: `${100 / STAGES.length}%` }}>
              <span className={`text-[9px] tracking-wider transition-colors ${
                i === stageIdx ? "text-indigo-400" : i < stageIdx ? "text-white/40" : "text-white/18"
      ...
      {/* Active stage detail */}
      <div className="mx-5 mb-4 border border-white/[0.07] rounded-lg px-4 py-3 bg-white/[0.02] min-h-[54px]">
    ...
    <Layout>
      ...
      <section className="max-w-6xl mx-auto px-8 pb-28">
        <Motion>
          ...
          <p className="text-white/42 mb-10 max-w-xl">Every application decision is validated against fair lending law, data protection requirements, and product regulations — before a single outcome is delivered.</p>
        </Motion>
